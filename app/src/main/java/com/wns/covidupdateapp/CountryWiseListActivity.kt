...
import kotlinx.android.synthetic.main.activity_country_wise_list.*
import kotlinx.android.synthetic.main.activity_day_wise_list.*
import kotlinx.android.synthetic.main.activity_day_wise_list.recyclerView

class CountryWiseListActivity : AppCompatActivity() {

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.activity_country_wise_list)

        var countryName = intent.getStringExtra("country")

        println("country is "+countryName)

        filterQueryWise(countryName)
    ...
    fun filterQueryWise(dateStr:String){
        ...
        val query: Query = reference.child("country_wise").orderByChild("country_name").equalTo(dateStr)
